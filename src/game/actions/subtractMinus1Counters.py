...
class SubtractMinus1Counters(IAction):
    ...
    def execute(self):
        try:
            ...
            if self._minus1Counters <= 0:
                raise ValueError("Please provide a positive non-zero number to subtract minus 1 counters.")

            if card.minus1Counters - self._minus1Counters < 0:
                raise ValueError("Minus 1 counters can not be negative.")
            else:
